...
foreach ( $wpm_levels as $k => $v ) {
	...
	foreach(array('removeFromLevel', 'addToLevel') AS $option) {
		$wpm_levels[$k][$option] = is_array($wpm_levels[$k][$option]) ? array_keys($wpm_levels[$k][$option]) : array();
	}
}
...
printf( "\n<script type='text/javascript'>\nvar js_levels = %s;\nvar wpm_levels = %s;\nvar level_stats = %s;\nvar members_link = %s;\nvar wlm_shortcodes = %s;\nvar wlm_sender_default = %s;\n</script>\n", json_encode( $js_levels ), json_encode( $wpm_levels ), json_encode( $stats ), json_encode( $members_link ), json_encode( $wlm_shortcodes ), json_encode( $wlm_sender_default ) );
