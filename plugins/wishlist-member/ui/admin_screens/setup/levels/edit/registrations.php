...
<div class="row">
	<template class="wlm3-form-group">
		{
			label: '<?php _e( 'Member Registration Options', 'wishlist-member' ); ?>',
			type: 'select',
			...
			addon_left: 'Allow',
			tooltip: '<?php _e( 'This option will affect how a registration form functions. This is helpful when using a form that is only available for new members. It\\\'s also  helpful in a situation where forms are specifically intended for existing users. For example, upsells or upgrades.', 'wishlist-member' ); ?>',
		}
	</template>
</div>
<div class="row">
	<div class="col-xxxl-4 col-md-6 col-sm-8 col-xs-8">
		<template class="wlm3-form-group">
			{
				label : '<?php _e( 'Enable Registration Form Header/Footer', 'wishlist-member' ); ?>',
				name  : 'enable_header_footer',
				tooltip : '<?php _e( 'Enable this feature to customize the Header section that will be displayed above a registration form and / or the Footer section that will be displayed below a registration form. ', 'wishlist-member' ); ?>',
				value : '1',
				tooltip_size: 'md',
		...
		<template class="wlm3-form-group">
			{
				...
				tooltip : '<?php _e( 'todo', 'wishlist-member' ); ?>',
				style : 'width: 100%'
			}
		</template>
	</div>
	<div class="col-md-auto col-sm-2 pl-0 pr-0 edit-custom-regform">
		<a href="#" data-link="<?php echo admin_url('admin.php'); ?>?page=WishListMember&wl=advanced_settings/registration_forms/custom#editform-" class="btn -icon-only edit-custom-regform-btn" title="Edit">
	...
	<div class="row justify-content-lg-end">
		...
		<div class="col-md-auto col-sm-auto">
			<a href="#" class="btn -primary -condensed -no-icon create-form-button" title="Create Form">
				<span><?php _e( 'Create Form', 'wishlist-member' ); ?></span>
			</a>
...
<div class="row">
	<div class="col-md-12">
		<template class="wlm3-form-group">
			{
				label : '<?php _e( 'Grant Continued Access', 'wishlist-member' ); ?>',
				name  : 'isfree',
				value : '1',
				tooltip : '<?php _e( 'Grant Continued Access allows a Member to retain access to content they have been given access to AFTER they cancel their membership.', 'wishlist-member' ); ?>',
				type  : 'toggle-switch',
			}
		</template>
	</div>
...
<div class="row">
	<div class="col-md-12">
		<template class="wlm3-form-group">
			{
				label : '<?php _e( 'Prevent Member from Editing Pre-filled Information', 'wishlist-member' ); ?>',
				tooltip : '<?php _e( 'When a membership level is connected to a payment integration, the payment provider will pass the First Name, Last Name and Email Address from the payment to the registration form. Enabling this option will prevent the user from editing any information that was pre-filled on the registration form.', 'wishlist-member' ); ?>',
		...
		<template class="wlm3-form-group">
			{
				name : 'url',
				'data-initial' : '',
				addon_left : '<?php echo WLMREGISTERURL; ?>/',
