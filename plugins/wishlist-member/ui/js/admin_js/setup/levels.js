...
wlm3_screen_levels.prototype = {
	...
	show_form: function(level, save_only) {
		...
		var xd = moment(level.expire_date, WLM3VARS.js_date_format);
		...
		$('#levels-create [name=expire_date]').daterangepicker(
			{
				...
				startDate: xd.format(WLM3VARS.js_date_format),
				locale : {
	...
	_clone: function(e) {
		...
		$('<div/>').save_settings({
			data : level,
			on_done : function() {
				window.parent.location.hash = '#levels_access-' + level.id;
			}
		});
	},
	_new: function() {
	...
	init: function() {
		...
		$('#levels-create')
			...
			.on('change.wlm3levels', ':checkbox[name]:not(.modal-input), :checked[name]:not(.modal-input), input[name][type=text]:not(.modal-input, [name=name], [name=url], [name=expire], [name=expire_date]), [name=removeFromLevel], [name=addToLevel], [name=disableexistinglink], [name=custom_reg_form], [name=role]', {
				that: this
			}, this._save_changes)
		...
		if(window.parent.location.hash) {
			...
			if(action.match(/^(#levels_access|#levels_registrations|#levels_requirements|#levels_additional_settings|#levels_notifications)$/)) {
				var level = hash.join('-');
				if(level == 'new') {
					this._new();
				} else {
...
$(function() {
	...
	$('#header-footer .nav-tabs a.nav-link').on('shown.bs.tab', function() {
		var textarea = $($(this).attr('href') + ' textarea').first();
		...
		CodeMirror.fromTextArea(textarea[0], mode ).on('change', function(cm, obj) {
