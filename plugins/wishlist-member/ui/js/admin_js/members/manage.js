...
jQuery(function($){
	add_level_modal = new wlm3_modal( '#add-level-modal', schedule_user_level );
    ...
    $("#" +edit_user_modal.data.id).find(".edit-user-nav-link").on('click', function(){
    	if ( $(this).hasClass('for-pay-per-posts') ) {
    		$(this).closest(".modal-body").find(".pp-nav-link").first().trigger("click");
    	}
    });
});
...
var show_add_level_modal = function( e ) {
	...
	select.select2({theme:"bootstrap",placeholder: wlm.translate( 'Select Membership Levels' )});
...
var show_cancel_level_modal = function( e ) {
	...
	var select = $("#" +cancel_level_modal.data.id).find(".wlm-levels");
...
var show_uncancel_level_modal = function( e ) {
	...
	var select = $("#" +uncancel_level_modal.data.id).find(".wlm-levels");
	...
	if ( select.data('select2') ) select.select2('destroy');
	if ( !select.data('select2') ) select.select2({theme:"bootstrap",placeholder: wlm.translate( 'Select Membership Level' )});
	$("#" +uncancel_level_modal.data.id).find(".wlm-levels").val( $(this).attr("data-levelid") ).trigger('change.select2');
	reset_calendar();
}
