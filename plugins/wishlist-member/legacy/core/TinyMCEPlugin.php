	...
	class WLMTinyMCEPluginOnly {
		...
		function TMCE_InsertButton() {
			...
			if ( wlm_arrval($_GET, 'page') != $WishListMemberInstance->MenuID ) {
				...
				wp_enqueue_script( 'wishlist-member-tinymce-lightbox-js', $WishListMemberInstance->pluginURL . '/js/tinymce_lightbox.js', array('jquery'), $WishListMemberInstance->Version);
	        ...
	        foreach($specialcodes as $index=>$scodes){
	        	if(count($scodes) > 0){
	        		if(!is_integer($index)){
                        ...
                        $title = $scodes['title']; $value=$scodes['value'];$value2 = substr_replace($value, '/', 1, 0);
	    ...
	    */
	    function RegisterShortcodes($name,$shortcodes=array(),$mergecodes=array(),$wponly=0,$jsfunc=null,$specialcodes=array()){
			...
			if(is_array($shortcodes) && count($shortcodes) > 0) $code['shortcode'] = $shortcodes;
			if(is_array($mergecodes) && count($mergecodes) > 0) $code['mergecode'] = $mergecodes;
			if(is_array($specialcodes) && count($specialcodes) > 0) $code['special'] = $specialcodes;
			if ( $this->use_legacy ) {
				$this->codes[] = $code;
			} else {
	        ...
	        foreach ( $codes as $index=>$scodes ) {
	        	...
	        	if ( count( $scodes ) > 0 ) {
	        		if ( ! is_integer( $index ) ) {
	        			...
	        			$sub_sub = $this->TNMCE_GenerateCode( $scodes, $sclevel_count,$merge );
	        			if ( ! empty( $sub_sub ) ) {
		                	$sub_sub = "{" .implode("},\n{", $sub_sub ) ."}";
		                    $sub = "text:'{$index}',\nmenu:[{$sub_sub}]";
	        			}
                        ...
                        if ( $js_func ) {
                        	...
                        	$merge= isset($scodes['type']) &&  $scodes['type'] == 'merge' ? $scodes['type'] : "";
                        	if ( $merge ){
                        		$scode2 = substr_replace($value, '/', 1, 0);
