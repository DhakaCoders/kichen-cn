 ...
 * $LastChangedDate: 2018-01-24 22:26:26 +0800 (Wed, 24 Jan 2018) $
 */
if (!defined('ABSPATH') ) die();

if (!class_exists('WishListMemberSystemInfo')) {
	 ...
	 * @subpackage classes
	 */
	class WishListMemberSystemInfo {
		...
		function get_user_values() {
			...
			if ( ! empty( $current_user->roles ) )  $role = $current_user->roles[0];
		...
		function get_server_values() {
			...
			$openssl = "No";
			if ( extension_loaded('openssl') ) $openssl = OPENSSL_VERSION_TEXT ." (ver. " .OPENSSL_VERSION_NUMBER .")";
			...
			if ( extension_loaded('curl') ) {
				...
				if ( isset( $json->tls_version ) ) $curl .= " (using " .$json->tls_version .")";
				else $curl .= " (TLS not available)";
			}
