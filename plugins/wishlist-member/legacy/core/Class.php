...
STRING;
		...
		function WPWLKeyProcess() {
			...
			if ($WPWLKey == '****************' || $this->isURLExempted(strtolower(get_bloginfo('url')))) {
				$this->DeleteOption( 'LicenseRets', 'LicenseStatus', 'LicenseSubscription', 'LicenseExpiration' );
...
Thank you.", 'wishlist-member'), home_url()),
		...
		function SaveOption($option, $value, $enc = false) {
			...
			$x = $wpdb->replace($this->OptionsTable, $data, $where);
...
//			$import_name = get_class($import);
		...
		function __call($method, $args) {
			if (array_key_exists($method, (array) $this->imported_functions)) {
				$args = (array) $args;
