...
class WLMAPI2 {
	...
	function __construct( $request, $method = 'GET', $data = null ) {
		...
		if ( $external ) {
			...
			if ( $method == 'POST' && ( $_POST['____FAKE____'] == 'PUT' || $_POST['____METHOD_EMULATION____'] == 'PUT' ) ) {
				$method = 'PUT';
			}
			if ( $method == 'POST' && ( $_POST['____FAKE____'] == 'DELETE' || $_POST['____METHOD_EMULATION____'] == 'DELETE' ) ) {
				$method = 'DELETE';
			}
			/*
			 * set $data
		...
		switch ( $this->method ) {
			...
			case 'GET':
				...
				if ( empty( $member_id ) ) {
					...
					if ( is_multisite() ) {
						...
						if ( empty( $filter_sql ) ) {
							$filter_sql = "WHERE {$wpdb->users}.ID = {$wpdb->usermeta}.user_id AND meta_key = '{$blog_prefix}capabilities'";
						} else {
							$filter_sql = $filter_sql . "AND {$wpdb->users}.ID = {$wpdb->usermeta}.user_id AND meta_key = '{$blog_prefix}capabilities'";
						}
						$result = $wpdb->get_results( "SELECT {$__found_rows__} `ID` AS `id`, `user_login`, `user_email`, CONCAT('/members/',`ID`) AS `_more_` FROM {$wpdb->users}, $wpdb->usermeta  {$filter_sql} {$__limit__}", ARRAY_A );
					} else {
			...
			case 'POST':
				...
				if ( is_multisite() ) {
					...
					if ( empty( $user_pass ) ) {
						$pass_length = $WishListMemberInstance->GetOption( 'min_passlength' ) + 0;
						if ( ! $pass_length ) {
							$pass_length = 8;
						}
						$user_pass = wp_generate_password( $pass_length, false );
					}
			...
			case 'PUT':
				...
				if ( ! isset( $data['display_name'] ) ) {
					$data['display_name'] = trim(
						implode(
							' ',
							array(
								wlm_arrval( $data, 'first_name' ),
								wlm_arrval( $data, 'last_name' ),
							)
						)
					);
				...
				foreach ( $data as $meta => $value ) {
					if ( substr( $meta, 0, 7 ) == 'custom_' || $meta == 'wpm_login_limit' || $meta == 'wpm_registration_ip' || $meta == 'stripe_cust_id' ) {
				...
				$all_levels = $new_levels = $confirm_levels = $pending_levels = array();
				...
				if ( $this->data['SendMail'] || $this->data['SendMailPerLevel'] ) {
					...
					if ( ! empty( $new_levels ) ) {
						if ( $this->data['SendMailPerLevel'] ) {
							foreach ( $new_levels as $lkey => $email_macros['[memberlevel]'] ) {
								...
								$WishListMemberInstance->email_template_level = $lkey;
							...
							$WishListMemberInstance->send_email_template( 'registration', $member_id, $email_macros, null, null, true );
						}
					}
					...
					if ( ! empty( $confirm_levels ) ) {
						if ( $this->data['SendMailPerLevel'] ) {
							...
							$WishListMemberInstance->send_email_template( 'email_confirmation', $member_id, $email_macros, null, null, true );
						}
					}
						...
						if ( $free ) {
							if ( $this->data['SendMailPerLevel'] ) {
								foreach ( $confirm_levels as $lkey => $email_macros['[memberlevel]'] ) {
									...
									$WishListMemberInstance->email_template_level = $lkey;
									$WishListMemberInstance->send_email_template( 'require_admin_approval', $member_id, $email_macros ); // send to user
								}
							} else {
								$email_macros['[memberlevel]'] = implode( ', ', $free );
								$WishListMemberInstance->send_email_template( 'require_admin_approval', $member_id, $email_macros, null, null, true ); // send to user
							}
						}
						...
						if ( $paid ) {
							if ( $this->data['SendMailPerLevel'] ) {
								foreach ( $paid as $lkey => $email_macros['[memberlevel]'] ) {
									...
									$WishListMemberInstance->email_template_level = $lkey;
									$WishListMemberInstance->send_email_template( 'require_admin_approval_paid_admin', $member_id, $email_macros, $WishListMemberInstance->GetOption( 'email_sender_address' ) ); // send to admin
								}
							} else {
								$email_macros['[memberlevel]'] = implode( ', ', $paid );
								$WishListMemberInstance->send_email_template( 'require_admin_approval_paid_admin', $member_id, $email_macros, $WishListMemberInstance->GetOption( 'email_sender_address' ), null, true ); // send to admin
							}
						}
					}
					...
					if ( $this->data['SendMailPerLevel'] ) {
						foreach ( $all_levels as $lkey => $email_macros['[memberlevel]'] ) {
							// skip sending of per level email if SendMailPerLevel is an array and the current level is not in the array
							if ( is_array( $this->data['SendMailPerLevel'] ) && ! in_array( $lkey, $this->data['SendMailPerLevel'] ) ) {
								continue;
							}
							...
							$WishListMemberInstance->email_template_level = $lkey;
							$WishListMemberInstance->send_email_template( 'admin_new_member_notice', $member_id, $email_macros, $WishListMemberInstance->GetOption( 'email_sender_address' ) );
						}
					} else {
						$email_macros['[memberlevel]'] = implode( ', ', array_unique( $all_levels ) );
						$WishListMemberInstance->send_email_template( 'admin_new_member_notice', $member_id, $email_macros, $WishListMemberInstance->GetOption( 'email_sender_address' ), null, true );
					}
				}
				$this->method = 'GET';
