...
if ( ! class_exists('WLM_Drip_Api') ) {
	include_once( $this->pluginDir . '/extlib/wlm_drip/Drip_API.class.php' );
}

if ( !class_exists('WLM_AUTORESPONDER_DRIP2_INIT') ) {

	class WLM_AUTORESPONDER_DRIP2_INIT {
	    ...
	    function load_hooks() {
			if ( $this->drip_api ) {
				...
				add_action( 'wishlistmember_uncancel_user_levels', array( $this, 'ReregUserTagsHookQueue' ), 99, 2 );
	    ...
	    function create_or_update_subscriber( $account_id, $data ) {
			...
			$params['custom_fields'] = array( 'name' => "{$data['first_name']} {$data['last_name']}", 'first_name' => $data['first_name'], 'last_name' => $data['last_name'], 'username' => $data['username'] );
			try {
				$ret = $this->drip_api->create_or_update_subscriber( $params );
				if ( $ret === false ) return array("errstr"=>"Error:" .$this->drip_api->get_error_message(), "errno"=> $this->drip_api->get_error_code() );
			} catch ( Exception $e ) {
				return array("errstr"=>"Error:" .$e->getMessage(), "errno"=>1 );
			}
			return true;
	    }
	    ...
	    function tag_untag_subscriber( $account_id, $email, $tags, $apply = true  ) {
			...
			foreach ( $tags as $tag ) {
				try{
					if ( $apply ) {
						$ret = $this->drip_api->tag_subscriber( array( "account_id"=>$account_id, "email"=>$email, "tag"=>trim( $tag ) ) );
					} else {
						$ret = $this->drip_api->untag_subscriber( array( "account_id"=>$account_id, "email"=>$email, "tag"=>trim( $tag ) ) );
					}
					if ( $ret !== true ) {
			...
			if ( !empty( $errstr ) ) {
				return array("errstr"=> $errstr, "errno"=>$errno );
			}
			return true;
	    }
	    ...
	    function create_event( $account_id, $email, $action, $levelid ) {
	    	...
	    	if ( !isset( $events[$action] ) ) return array( "errstr"=>"Invalid action for event", "errno"=>1 );
		...
		function UpdateProfile( $user_id, $old_data ) {
			...
			if ( $sub ) {
				...
				if ( $old_data->user_email != $user_data->user_email ) $email_changed = true;
				...
				if ( count( $cfields ) > 0 || $email_changed ) {
					...
					try {
						$ret = $this->drip_api->create_or_update_subscriber( $params );
						if ( $ret === false ) $msg = "Drip2Error:" .$this->drip_api->get_error_code() ."-" .$this->drip_api->get_error_message();
					} catch ( Exception $e ) {
						$msg = "Drip2Error:" .$e->getMessage();
					}
					if ( !empty( $msg ) ) trigger_error( $msg );
				}
			}
		}
		...
		function processTags( $levels, $action, $data ) {
			global $WishListMemberInstance;
			if ( !isset( $WishListMemberInstance ) ) return array( "errstr"=> "WishList Member instance not found.", "errno"=>1 );
			if ( !$this->drip_api ) return array( "errstr"=> "Unable to process tags. No API Connection.", "errno"=>1 );
			$levels = (array) $levels;
			if ( count( $levels ) <= 0 ) return array("errstr"=>"No Levels Found","errno"=>1);//no levels, no need to continue
			if ( !isset( $data['user_email'] ) || empty( $data['user_email'] ) ) return array( "errstr"=>"Email address not found", "errno"=>1 );
			if ( ! in_array( $action, array( 'add', 'cancel', 'rereg', 'remove' ) ) ) return array( "errstr"=>"Invalid action", "errno"=>1 );

			$account_id = isset( $this->drip2_settings['account'] ) ? $this->drip2_settings['account'] : false;
			$account_id = !empty( $account_id ) ? $account_id : false;
			if ( !$account_id ) return array( "errstr"=>"No account configured", "errno"=>1 );
			...
			foreach( (array) $levels as $level ) {
				...
				if ( !$apply_tags && !$remove_tag && !$record_event ) continue; //skip the rest of the loop
				...
				if ( !$record_checked ) { //lets check the email if its in drip
					...
					if ( !$x ) {
						...
						if ( $x !== true ) return $x; //an error occured when adding record to drip
					}
					$record_checked = true;
				}
...
$Drip2InitInstance = new WLM_AUTORESPONDER_DRIP2_INIT();
$Drip2InitInstance->load_hooks();