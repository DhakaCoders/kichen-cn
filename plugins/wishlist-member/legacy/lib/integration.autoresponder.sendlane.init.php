<?php
/*
 * Sendlane AutoResponder Init File
 * Sendlane Site: http://sendlane.com/
 * Original Integration Author : Fel Jun Palawan
 * Version: $Id$
 */
if ( !class_exists('WLM_Sendlane') ) {
	require_once( $this->pluginDir . '/extlib/wlm-sendlane.php' );
}

if ( !class_exists('WLM_AUTORESPONDER_SENDLANE_INIT') ) {

	class WLM_AUTORESPONDER_SENDLANE_INIT {
		private $settings = "";
		...
		function __construct() {
			...
			if ( isset( $WishListMemberInstance ) && class_exists( 'WLM_Sendlane' ) ) {
				...
				$this->settings = isset($ar["sendlane"]) ? $ar["sendlane"] : false;
				...
				if ( $this->settings && !empty( $this->settings['api_key'] ) && !empty( $this->settings['api_hash'] ) && !empty( $this->settings['subdomain'] ) ) {
					...
					if ( isset( $lists_ret['error'] ) )  $this->sendlane_api = false;
				}
			}
		}
		...
		function processTags( $levels, $action, $data ) {
			...
			if ( !$this->sendlane_api ) return array( "errstr"=> "Unable to process tags. No API Connection.", "errno"=>1 );
			$levels = (array) $levels;
			...
			foreach( (array) $levels as $level ) {
				...
				if ( $list_add ) {
					...
					if ( isset( $ret["error"] ) ) $error["list_add"] = $ret["error"]["messages"];
				}
				...
				if ( $list_remove ) {
					...
					if ( isset( $ret["error"] ) && !isset( $ret["error"][406] ) ) $error["list_remove"] = $ret["error"]["messages"];
				}

				if ( !$apply_tags && !$remove_tag ) continue; //skip the rest of the loop
				...
				if ( $apply_tags ) {
					...
					if ( isset( $ret["error"] ) ) $error["apply_tag"] = $ret["error"]["messages"];
				}
				...
				if ( $remove_tag ) {
					...
					if ( isset( $ret["error"] ) ) $error["remove_tag"] = $ret["error"]["messages"];
				}

				if ( count( $error ) ) $errors[ $level ] = $error;
			}
			return count( $errors ) ? maybe_serialize($errors) : true; //success
		}
		...
		function ifarProcessQueue( $recnum = 10, $tries = 3 ){
			if ( !$this->sendlane_api ) return;
			$WishlistAPIQueueInstance = new WishlistAPIQueue;
...
$SendlaneInitInstance = new WLM_AUTORESPONDER_SENDLANE_INIT();
$SendlaneInitInstance->load_hooks();