 ...
 * Version: $Id: integration.shoppingcart.stripe.php 6535 2019-12-16 16:00:12Z mike $
 */
...
if (!class_exists('WLM_INTEGRATION_STRIPE')) {
	...
	class WLM_INTEGRATION_STRIPE {
					...
					if(count($cust->subscriptions->data) > 1) {
						...
						foreach($cust->subscriptions->data as $d) {
							if($d->plan->id == $plan_id) {
								...
								if($at_period_end) {
									$update = WLMStripe\Subscription::update($sub_id, array(
		...
		public function sync($data = array()) {
			...
			if($obj->id == 'evt_00000000000000') die( "\n" );
			...
			try {
				...
				if($data['subscription']) {
					...
					if($subs->latest_invoice->payment_intent->status == 'requires_action') {
						$this->process_sca_auth($data, $subs->latest_invoice->payment_intent->client_secret, $cust->id, $stripe_plan, '', 'charge_existing', $current_user->ID); // If card needs authentication then let's initiate SCA popup
					}
			...
			try {
				if($data['subscription']) {
					...
					if($subs->latest_invoice->payment_intent->status == 'requires_action') {
						...
						$this->process_sca_auth($data, $subs->latest_invoice->payment_intent->client_secret, $cust->id, $stripe_plan, '', 'charge_new'); 
					}
		...
		public function invoices($data) {
			...
			try {
				...
				$inv = WLMStripe\Invoice::all(array('count' => 100, 'customer' => $cust_id));
				$invoices = array();
				if (!empty($inv['data'])) {
