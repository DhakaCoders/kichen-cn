...
if ( ! class_exists( 'WLM_OTHER_INTEGRATION_ZAPIER' ) ) {
	class WLM_OTHER_INTEGRATION_ZAPIER {
		...
		public static function Zapier() {
			...
			if ( strpos( $_SERVER['QUERY_STRING'], '/zapier/' ) !== false ) {
				$url_parts = explode( '/zapier/', $_SERVER['QUERY_STRING'] );
		...
		public static function user_poll_test( $data ) {
			...
			if ( in_array(
				...
				$data['levels'] = array_values( ( new WishListMemberUser( $rand_id, false ) )->Levels );
				if ( $data['levels'] ) {
		...
		public static function add_member( $data ) {
			...
			$user['levels'] = array_values( ( new WishListMemberUser( $user['user_id'], false ) )->Levels );
		...
		public static function _level_management( $data, $action ) {
			...
			if ( empty( $user ) ) {
				return array( '', 404 ); // return if email does not match a user
			}
			...
			// include levels to be returned
			$user['levels'] = array_values( ( new WishListMemberUser( $user_id, false ) )->Levels );
		...
		public static function _wlmhook_add_levels( $user_id, $levels ) {
			...
			foreach ( $levels as $level ) {
				...
				$data['levels']     = array_values( ( new WishListMemberUser( $user_id, false ) )->Levels );
				self::_zap( array( $data ), $event );
