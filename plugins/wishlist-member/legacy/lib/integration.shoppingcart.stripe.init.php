...
if (!class_exists('WLM_Stripe_ShortCodes')) {
	class WLM_Stripe_ShortCodes {
		public function __construct() {
			...
			if(in_array($pagenow, array('post.php', 'post-new.php', 'admin.php'))) {
				...
				foreach($levels as $i => $l) {
					$wlm_shortcodes[] = array('title' => $l['name'] . $str , 'value' => sprintf("[wlm_stripe_btn sku=%s button_label=\"Join %%level\" pay_button_label=\"Pay\"]", $i));
				}
		...
		function add_stripe_field ( $custom_fields, $userid ) {
			...
			if(empty($stripeapikey) && empty($stripeapikey))  return $custom_fields;
		...
		function save_stripe_field( $data ) {
			...
			if ( ! isset( $user_custom_fields['stripe_cust_id'] ) ) return;
		...
		public function wlm_stripe_profile($atts) {
			...
			$default_atts = array('levels' => '','include_posts' => 'yes', 'hide_cancel_button' => 'no' );
								...
								stripe.createToken(card$sku, cardData).then(function(result) {
								    if (result.error) {
								      ...
								      errorElement.textContent = result.error.message;
								      	...
								      	// From old
								     	ui.find('.card-errors-$sku').html( '<p>' + result.error.message  + '</p>');
...
<?php
		...
		public function generate_stripe_form($atts, $content) {
			...
			if ($settings['subscription']) {
				try {
					WLMStripe\WLM_Stripe::setApiKey($stripeapikey);
					$plan = WLMStripe\Plan::retrieve($settings['plan']);
