...
<script type="text/javascript">
	...
	var wlm_displayed_tab = <?php echo json_encode( $this->GetOption( 'wlm3_postpage_displayed_tab' ) ); ?>;

	var toggle_markup = '<div class="switch"><div class="switch-left"><div class="form-group"><label class="switch-light switch-wlm"><input name="_toggle_name_" value="1" type="checkbox" _toggle_checked_><span><span><img src="<?php echo $this->pluginURL3; ?>/ui/images/outline-check-24px.svg" alt=""></span><span><img src="<?php echo $this->pluginURL3; ?>/ui/images/baseline-close-24px.svg" alt=""></span><a target="_parent"></a></span></label></div></div></div>';
    ...
    jQuery('.wlm-postpage-apply').click(function() {
		...
		jQuery('#wlm3-ppp-search-button').click(function(){
			...
			var $search = $search_by == 'by_user' ? jQuery('#wlm_user_search_input').val() : jQuery('#wlm_level_search_input').val();
			...
			jQuery.post(
				...
				function(result) {
					...
					if(result.users.length) {
						...
						$tbody.html('<tr><td colspan="5"><p><?php _e('No results found', 'wishlist-member'); ?></p></td></tr>');
					}
				}
			);
		});
