...
// if version is 6649 then we display errors
global $wlm_globalrev;
$wlm_globalrev = '6649' == '{'.'GLOBALREV}';
if(!$wlm_globalrev) {
...
require_once(dirname(__FILE__) . '/core/WishListNavMenu.php');
...
if (!class_exists('WishListMember')) {
	...
	class WishListMember extends WishListMemberPluginMethods {
		...
		function __construct() {
			...
			if (substr($this->Version, -11) == '6649' && !defined('WLMEMBER_EXPERIMENTAL')) {
		...
		function Activate() {
			...
			if( is_array( $wpm_levels ) && count( $wpm_levels ) ) {
				foreach ($wpm_levels AS $key => $level) {
					...
					if (empty($level['upgradeTo']) OR empty($level['upgradeMethod']) OR ($level['upgradeMethod'] == 'MOVE' && !((int) $level['upgradeAfter']) && empty($level['upgradeSchedule']))) {
...
// sc;
		...
		function Init() {
			...
			// This is for WLM's Login shortcode and widget as COOKIES are needed for the login process to work.
		...
		function PreparePostPageOptions() {
			...
			foreach ( $post_types AS $post_type ) {
				if ( $post_type == 'attachment' ) {
					...
					add_meta_box( 'wlm_postpage_metabox', ' ', array( &$WishListMemberInstance, 'PostPageOptions' ), $post_type );
				}
			}
		}
		...
		function Login() {
			...
			if ($this->LoginCounter($user)) {
				...
				if (wlm_arrval($_POST, 'redirect_to') == 'wishlistmember' || !$user->caps['administrator']) {
					...
					$url = apply_filters('wlm_after_login_redirect', $url, $level);
		...
		function Process () {
			...
			// If Buddypress is not active, do normal 404 checking.
			    if(is_404()) return;
			}    
			...
			if (!$wpm_current_user->ID) {
				...
				if (!$canviewpage && is_page()) {
					...
					$wronglevel = true;
				} elseif ((!$canviewcategory && $wlm_is_category) OR is_tax()) {
		...
		function MailSubject( $wp_mail ) {
			$this->MailSubject = trim( $this->MailSubject );
...
STRING;
		...
		function ProcessApiQueue($limit = 10,$tries=3) {
			...
			$activate_thirdparty_providers = (array) $this->GetOption('active_other_integrations');
			if( in_array( 'gotomeetingapi', $activate_thirdparty_providers ) ) {
				include_once($this->pluginDir . '/lib/integration.webinar.gotomeetingapi.php');
				// Process GotoWebinar Refresh token if the integration is enabled.
				if(class_exists("WishListMemberWebinarIntegrationGotowebinarApi")){
					$WishListMemberWebinarIntegrationGotowebinarApi = new WishListMemberWebinarIntegrationGotowebinarApi;
					if (isset($WishListMemberWebinarIntegrationGotowebinarApi) && method_exists($WishListMemberWebinarIntegrationGotowebinarApi, 'refreshtoken')) {
						$WishListMemberWebinarIntegrationGotowebinarApi->refreshtoken($limit);
					}
				}
			}
		}
		...
		function DoSequentialForUser($id, $sync_membership = false) {
			...
			if ( $last_run && $last_run >= $fourteen_minutes ) { return $last_run; }
		...
		function ExpiringMembersNotification() {
			...
			foreach(array('admin','user') AS $type) {
				foreach ($expiring_users[$type] as $u) {
					...
					if(empty($users[$uid])) {
						$users[$uid] = new WishListMemberUser($uid, true);
					}
		...
		*/
		function AddEditorLightBoxMarkup () {
			...
			if ( current_user_can('edit_posts') && current_user_can('edit_pages') && is_admin() ) {
				$this->tinymce_lightbox_files[] = $this->pluginDir ."/resources/lightbox/tinymce-mergecodes.php";
				ob_start();
			...
			if ($unsubscribe_expired) {
				...
				foreach($expired_members as $key => $expired_ids) {
					...
					foreach($expired_ids as $expired_id) {
						...
						if($unsubscribe_expired) {
							$unsubscribe_expired = maybe_unserialize($unsubscribe_expired);
							...
							if (!in_array($key, $unsubscribe_expired)) {
								...
								$this->Update_UserMeta($expired_id, 'unsubscribe_expired_members_processed', maybe_serialize( (array) $unsubscribe_expired ));
							}

						} else {
							$user = $this->Get_UserData($expired_id);
							$this->ARUnsubscribe($user->first_name, $user->last_name, $user->user_email, $key);
							$this->Update_UserMeta($expired_id, 'unsubscribe_expired_members_processed', maybe_serialize( (array) $key ));
						}
					}
		...
		/**
		 * After auto registration redirect
		 */
		public function after_autoreg_redirect() {
			if('CREATE' === wlm_arrval($_POST, 'cmd')) {
				return;
			} else {
				$url = $this->GetAfterRegRedirect($_POST['wpm_id']);
				header('Location: ' . $url);
			}
			exit;
		}
			      	...
			      	if(!$update) {
				        ...
				        if(!is_admin()) {
				        	...
				        	if(isset($_POST['action']) && ($_POST['action'] == 'bbp-new-topic' || $_POST['action'] == 'bbp-new-reply')) {
								...
								if($post_type == 'reply') {
									$lvls_that_have_access = $this->GetContentLevels('topic', $_POST['bbp_topic_id']);	
								} else {
		...
		function shortcodes_init() {
			...
			//shortcodes array
			$wlm_shortcodes[] = array('title' => 'First Name', 'value' => '[wlm_firstname]');
			$wlm_shortcodes[] = array('title' => 'Last Name', 'value' => '[wlm_lastname]');
			$wlm_shortcodes[] = array('title' => 'Email', 'value' => '[wlm_email]');
			$wlm_shortcodes[] = array('title' => 'Username', 'value' => '[wlm_username]');
			$wlm_shortcodes[] = array('title' => 'Membership Levels', 'value' => '[wlm_memberlevel]');
			$wlm_shortcodes[] = array('title' => 'Pay Per Posts', 'value' => '[wlm_userpayperpost]');
			$wlm_shortcodes[] = array('title' => 'Login Form', 'value' => '[wlm_loginform]');
			$wlm_shortcodes[] = array('title' => 'Login URL', 'value' => '[wlm_loginurl]');
			$wlm_shortcodes[] = array('title' => 'Profile Form', 'value' => '[wlm_profileform]');
			$wlm_shortcodes[] = array('title' => 'Profile URL', 'value' => '[wlm_profileurl]');
			$wlm_shortcodes[] = array('title' => 'RSS Feed', 'value' => '[wlm_rss]');

			foreach ((array) $wpm_levels AS $level) {
				if (strpos($level['name'], '/') === false) {
					$wlm_shortcodes[] = array('title' => "Join Date: {$level['name']}", 'value' => "[wlm_joindate {$level['name']}]");
					$wlm_shortcodes[] = array('title' => "Expiration Date: {$level['name']}", 'value' => "[wlm_expiration {$level['name']}]");
				}
			}

			$wlm_shortcodes[] = array('title' => 'Content Levels', 'value' => '[wlm_contentlevels type="comma" link_target="_blank" class="wlm_contentlevels" show_link="1" salespage_only="1"]');
			...
			$wlm_shortcodes[] = array('title' => 'Website', 'value' => '[wlm_website]');
			$wlm_shortcodes[] = array('title' => 'AOL Instant Messenger', 'value' => '[wlm_aim]');
			$wlm_shortcodes[] = array('title' => 'Yahoo Instant Messenger', 'value' => '[wlm_yim]');
			$wlm_shortcodes[] = array('title' => 'Jabber', 'value' => '[wlm_jabber]');
			$wlm_shortcodes[] = array('title' => 'Biography', 'value' => '[wlm_biography]');
			$wlm_shortcodes[] = array('title' => 'Company', 'value' => '[wlm_company]');
			$wlm_shortcodes[] = array('title' => 'Address', 'value' => '[wlm_address]');
			$wlm_shortcodes[] = array('title' => 'Address 1', 'value' => '[wlm_address1]');
			$wlm_shortcodes[] = array('title' => 'Address 2', 'value' => '[wlm_address2]');
			$wlm_shortcodes[] = array('title' => 'City', 'value' => '[wlm_city]');
			$wlm_shortcodes[] = array('title' => 'State', 'value' => '[wlm_state]');
			$wlm_shortcodes[] = array('title' => 'Zip', 'value' => '[wlm_zip]');
			$wlm_shortcodes[] = array('title' => 'Country', 'value' => '[wlm_country]');
		...
		function _privacy_user_request_mergecodes( $data ) {
			...
			foreach($data AS $key => $value) {
				$merge_codes['[' . $key . ']'] = sprintf( '###%s###', strtoupper( $value ) );
			}
			return $merge_codes;
		}
		...
		function privacy_personal_data_exporter( $email_address ) {
			...
			$user = new WishListMemberUser( $email_address, true );
			unset( $user->WL );
			if( ! $user ) {
				return array(
					'data' => $user,
