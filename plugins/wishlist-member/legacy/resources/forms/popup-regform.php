...
<div id="regform-<?php echo $id ?>" class="wlm-regform container-rs regform <?php echo $additional_classes; ?>">
	<div class="regform-container row-rs">
		...
		<?php if(wlm_arrval($_GET, 'status') == 'fail') : ?>
			<div class="regform-error">
				<p>
					<?php if (isset($_GET['status']) && $_GET['status'] == 'fail') _e("An error has occured while processing payment, please try again", "wishlist-member") ?>
			...
			foreach($fields as $f) {
				...
				switch ($f['type']) {
					...
					case 'select':
						...
						echo sprintf('<div class="txt-fld %6$s %1$s"><label for="%1$s">%2$s</label><select id="" class="regform-%1$s %5$s" name="%1$s" placeholder="%3$s">%4$s</select></div>',
						$f['name'],
						$f['label'],
						$f['placeholder'] ?: $f['label'],
						implode('', $options),
	...
	',
							...
							if(in_array('cc_cvc', $cc_has)) {
								echo sprintf('<div class="txt-fld code col-2 col-6-sm"><label>%1$s</label><input autocomplete="false" placeholder="CVC" maxlength="4"  class="regform-cvc" name="cc_cvc" type="text" /></div>',
				...
				<?php if(isset($data['sc'])): 
						switch($data['sc']){
							case 'stripe': // Add Description of payment and separate button from the amounnt description
								...
								$interval_count = $data['sc_details']['plan_details']->interval_count; 
								$interval = $data['sc_details']['plan_details']->interval; 
								if($data['sc_details']['is_subscription']) { // If subscription then include the interval of the payment
									$xamt = number_format( $data['sc_details']['amt'], 2, '.', '' );
									$interval = ucwords( strtolower( $interval ) );
									if($interval_count == 1) {
										$pay_desc = sprintf( '%s %s Every %s', $stripe_cur, $xamt, $interval );
									} else {
										$pay_desc = sprintf( '%s %s Every %d %ss', $stripe_cur, $xamt, $interval_count, $interval );
									}
								} else {
									$pay_desc = $stripe_cur.' '.number_format($data['sc_details']['amt'], 2, '.', '');
								}
								echo '
									<div style="float:left;"><button class="regform-button" name="regform-button">'.$data['sc_details']['panel_btn_label'].'
									</button></div>
									<div class="btn-fld-info">'. $pay_desc.'</div>
									<button style="display:none" name = "product_price" id ="amount" value="">'.$data['sc_details']['panel_btn_label'].' </button>	
									';
							break;
						}
					?>
