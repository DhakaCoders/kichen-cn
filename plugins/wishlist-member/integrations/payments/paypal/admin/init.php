...
if ( $data->eotcancel ) {
	$data->eotcancel = maybe_unserialize( $data->eotcancel );
} else {
	$data->eotcancel = array();
}

$data->subscrcancel = (array) maybe_unserialize( $this->GetOption( 'subscrcancel' ) ) + array_combine( array_keys( $wpm_levels ), array_fill( 0, count( $wpm_levels ), 1 ) );
