...
if ( $data->paypaleceotcancel ) {
	$data->paypaleceotcancel = maybe_unserialize( $data->paypaleceotcancel );
} else {
	$data->paypaleceotcancel = array();
}

$data->paypalecsubscrcancel = (array) maybe_unserialize( $this->GetOption( 'paypalecsubscrcancel' ) ) + array_combine( array_keys( $wpm_levels ), array_fill( 0, count( $wpm_levels ), 1 ) );
