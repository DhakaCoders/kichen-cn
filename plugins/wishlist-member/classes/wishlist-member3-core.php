...
if (!class_exists('WishListMember3_Core')) {
	...
	class WishListMember3_Core extends WishListMember {
		...
		function constructor3($pluginfile, $sku, $menuid, $title, $link) {
			...
			$this->pagination_items	= array(10,25,50,100,200);
			...
			if ($cver != $this->Version) {
				...
				$this->create_rollback_version( $cver );
		 ...
		 */
		function admin_page() {
			include_once $this->pluginDir3 . '/helpers/before_iframe.php';
			$this->user_interface();
			include_once $this->pluginDir3 . '/helpers/after_iframe.php';
		}
		...
		function tooltip($tooltip, $size = '', $return = false) {
			$size = trim( $size );
			if( empty( $size ) ) {
				$size = 'sm';
			}
			$text = '<a href="#" data-size="%s" class="wlm-icons help-icon" title="%s">help</a>';
			if($return) {
				return sprintf( $text, $size, htmlentities( $tooltip, ENT_QUOTES ) );
			} else {
				printf( $text, $size, htmlentities( $tooltip, ENT_QUOTES ) );
			}
		}
