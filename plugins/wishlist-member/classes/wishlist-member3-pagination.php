...
class WishListMember3_Pagination {
	public $items;
	public $per_page;
	public $pages;
	public $current;
	...
	function __construct( $items, $per_page, $current, $variable, $url, $per_page_options = [] ) {
		...
		$this->pages = ceil( $items / $per_page );
		...
		$this->from = ( $this->current - 1 ) * $per_page + 1;
		if( $this->from > $items ) $this->from = $items;

		$to = $this->from + $per_page - 1;
		...
		$this->per_page_options	= !is_array( $per_page_options ) ? [ 10, 25, 50, 100, 250, 500 ] : $per_page_options;
	}
	...
	private function get_range_markup( $per_page_query_var = 'howmany' ) {
		...
		foreach( $this->per_page_options AS $x ) {
			$per_page_options_markup .= sprintf( '<a class="dropdown-item" target="_parent" href="%s">%d</a>', add_query_arg( $per_page_query_var, $x, $this->url ), $x );
		}
	...
	private function get_input_markup() {
		$markup = sprintf( '<input type="text" value="%d" class="form-control text-center pagination-pagenum">', $this->current );
		return $markup;
	}
}
