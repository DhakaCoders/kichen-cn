...
if (!class_exists("WishListMember3_Actions")) {
	class WishListMember3_Actions extends WishListMember3_Core {
		...
		function process_admin_actions() {
			if ( !isset( $_POST['WishListMemberAction'] ) || empty( $_POST['WishListMemberAction'] ) ) return;
		...
		function process_admin_ajax_actions($action, $data) {
			if ( is_admin() ) {
				switch ($action) {
					...
					case 'save':
						$data = (array) $data;
						if ( count( $data ) < 0 ) return array("success"=> false, "msg"=> __( 'Empty request', 'wishlist-member' ), "msg_type"=>"danger");
						...
						return array("success"=> true, "msg"=>__( 'Saved', 'wishlist-member' ), "msg_type"=>"success", "data" => $data);
						break;
					case 'save_user_meta':
						$data = (array) $data;
						if ( count( $data ) < 0 ) return array("success"=> false, "msg"=>__( 'Empty request', 'wishlist-member' ), "msg_type"=>"danger");
					...
					default:
						return array("success"=> false, "msg"=>__( 'Invalid request', 'wishlist-member' ), "msg_type"=>"danger");
				}
			} else {
				return array("success"=> false, "msg"=>__( 'Unauthorized request', 'wishlist-member' ), "msg_type"=>"danger");
			}
		}
		...
		function reset_settings($data) {
			...
			$this->ResetSettings();
			if ( isset( $_POST['err'] ) ) {
				return array("success"=> false, "msg"=>$_POST['err'], "msg_type"=>"danger", "data"=>$data );
			} else {
				return array("success"=> true, "msg"=>$_POST['msg'], "msg_type"=>"success", "data"=>$data );
			}
		}
		...
		function delete_backup($data) {
			...
			return array("success"=> true, "msg"=>$_POST['msg'], "msg_type"=>"success", "data"=>$data , "details" => $details );
		}
		...
		function create_backup($data) {
			...
			if ( $ret === false ) {
				...
				return array("success"=> true, "msg"=>$_POST['msg'], "msg_type"=>"success", "data"=>$data, "files"=>$backup_files );
			}
		}

		function massmove_members($data) {
			$wpm_levels = $this->GetOption('wpm_levels');
			if ( !in_array( $data['operation'], array("move","add") ) ) return array("success"=> false, "msg"=>__('Invalid action','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
			if ( !array_key_exists( $data["to_levelid"], $wpm_levels ) ) return array("success"=> false, "msg"=>__('Invalid to membership level','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
			...
			if ( $data['operation'] == "add" ) {
				...
				if ( !array_key_exists( $data["from_levelid"], $wpm_levels ) ) return array("success"=> false, "msg"=>__('Invalid from membership level','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
				foreach ( $ids AS $id ) {
					$levels = $this->GetMembershipLevels( $id );
			...
			return array("success"=> true, "msg"=>__('Members have been processed','wishlist-member'), "msg_type"=>"success", "data"=>$data );
		}
		...
		function get_level_memberids($data) {
			...
			return array( "ids"=> $ids, "data" =>$data );
		}
		...
		function save_sequential($data) {
			...
			if ( isset( $_POST['sequential_err_msg'] ) ) {
				return array("success"=> false, "msg"=>$_POST['sequential_err_msg'], "msg_type"=>"danger", "data" => $_POST );
			} else {
				return array("success"=> true, "msg"=>$_POST['msg'], "msg_type"=>"success", "data" => $_POST );
			}
		}
		...
		function remove_savedsearch($data) {
			if ( isset( $data['name'] ) && !empty($data['name']) ) {
				...
				return array("success"=> true, "msg"=>__('Saved Search Removed','wishlist-member'), "msg_type"=>"success", "data" => $data );
			} else {
				return array("success"=> false, "msg"=>__('Invalid Saved Search name','wishlist-member'), "msg_type"=>"danger", 'data'=>$data );
			}
		}
		...
		function generate_password($data) {
			...
			$passmin += 0 ;
			if ( ! $passmin || $passmin < 14 ) $passmin = 14;

			//always generate a strong password
			$pass = wp_generate_password( $passmin, true );
			while ( !wlm_check_password_strength( $pass ) ) {
				$pass = wp_generate_password( $passmin, true );
			}

			if ( $pass ) {
				return array("success"=> true, "msg"=>__('Password generated','wishlist-member'), "msg_type"=>"success", "data" => $pass );
			} else {
				return array("success"=> false, "msg"=>__('Unable to generate password','wishlist-member'), "msg_type"=>"danger");
			}
		}
		...
		function add_remove_blacklist ( $data ) {
			...
			return array("success"=> true, "msg"=>$message, "msg_type"=>"success");
		}
		...
		function save_user_meta( $data ) {
			if ( !isset( $data['userid'] ) ) return array("success"=> false, "msg"=>__('Invalid Member','wishlist-member'), "msg_type"=>"danger");
			$userid = $data['userid'];
			unset($data['userid']);
			foreach ($data AS $option=>$value) {
				$this->Update_UserMeta($userid, $option, $value);
			}
			return array("success"=> true, "msg"=>__('Saved','wishlist-member'), "msg_type"=>"success", "data" => $data, "userid" => $userid );
		}
		...
		function schedule_user_level( $data ) {
			...
			if ( count( $userids ) <= 0 ) return array("success"=> false, "msg"=>__('No Member selected','wishlist-member'), "msg_type"=>"danger");
			if ( $action == 'unschedule_user_all' ) {
				...
				return array("success"=> true, "msg"=>sprintf( __("Sequential set to %s",'wishlist-member'), $on ), "msg_type"=>"success", "data"=>$return_data);
				...
				$return_data = array(1,1); //just to trigger the refresh on js side
				return array("success"=> true, "msg"=>sprintf( __("Selected members have been %s Email Broadcast.",'wishlist-member'), $sub_or_unsub ), "msg_type"=>"success", "data"=>$return_data);
				...
				if ( $post_type ) {
					...
					$return_data = array(1,1); //just to trigger the refresh on js side
					return array("success"=> true, "msg"=>__('Member Pay per post updated','wishlist-member'), "msg_type"=>"success", "data"=>$return_data);
				} else {
					return array("success"=> false, "msg"=>__('Invalid Post','wishlist-member'), "msg_type"=>"danger", "data"=>$data);
				}
			} else {
				if ( count( $wlm_levels ) <= 0 ) return array("success"=> false, "msg"=>__('No level selected','wishlist-member'), "msg_type"=>"danger");
				...
				foreach ( $wlm_levels as $level ) {
					if ( $action == 'add_user_level' ) {
						...
						if ( $sdate > $todays_date ) $action_msg = __("is scheduled to be added to the", 'wishlist-member');
						...
						$sdate = gmmktime(gmdate('H'), gmdate('i'), gmdate('s'), (int) $cdate_array[0], (int) $cdate_array[1], (int) $cdate_array[2]);
						if ( $sdate > $todays_date ) $action_msg = __("is scheduled to be removed from the", 'wishlist-member');
						...
						$cdate_array = explode('/', $schedule_date);
						$sdate = gmmktime(gmdate('H'), gmdate('i'), gmdate('s'), (int) $cdate_array[0], (int) $cdate_array[1], (int) $cdate_array[2]);
						if ( $sdate > $todays_date ) $action_msg = __("is scheduled to be moved to the", 'wishlist-member');
						...
						if ( !$status ) {
							foreach($userids as $m_id)
								$this->SendAdminApprovalNotification($m_id, $level);
						}
						...
						return array("success"=> false, "msg"=>__('Invalid Action','wishlist-member'), "msg_type"=>"danger");
					}
				}
			}
			...
			foreach ( $userids as $userid ) {
				...
				if ( count( $userids ) <= 1) {
		            $wlUser = new WishListMemberUser($userid);
			...
			$lvl_msg = "level";
			if ( count( $wlm_levels ) > 1 ) $lvl_msg = "levels";
			if ( count( $userids ) > 1 ) $return_data = array("success"=> true, "msg"=>sprintf( __("Selected members were %s %s",'wishlist-member'), $action_msg, $lvl_msg ), "msg_type"=>"success", "data"=>$return_data );
			else $return_data = array("success"=> true, "msg"=>sprintf( __("Member %s %s",'wishlist-member'), $action_msg, $lvl_msg ), "msg_type"=>"success", "data"=>$return_data, 'user_levels' => array_values($wlUser->Levels));
			if ( count( $userlevel_data ) > 0 ) $return_data["level_data"] = $userlevel_data;
			$return_data["x"] = $data;
			return $return_data;
		}
		...
		function add_remove_payperpost( $data ) {
			...
			if ( !$post ) return array("success"=> false, "msg"=>__('Invalid post','wishlist-member'), "msg_type"=>"danger", "data"=>$data);
			...
			if ( $data['operation'] == "add" ) {
				...
				if ( count( $return_data ) > 0 ) {
					if ( count( $return_data ) > 1 ) {
						return array("success"=> true, "msg"=>sprintf( __("Selected members has been given access to the %s",'wishlist-member'), $post["post_type"] ), "msg_type"=>"success", "data"=>$return_data );
					} else {
						return array("success"=> true, "msg"=>sprintf( __("Selected member has been given access to the %s",'wishlist-member'), $post["post_type"] ), "msg_type"=>"success", "data"=>$return_data );
					}
				} else {
					return array("success"=> false, "msg"=>__('Invalid member','wishlist-member'), "msg_type"=>"danger", "data"=>$data);
				}
			} else {
				$user = get_user_by('id', $data['userid']);
				if ( !$user ) return array("success"=> false, "msg"=>__('Invalid member','wishlist-member'), "msg_type"=>"danger", "data"=>$data);
				$post['userid'] = $user->ID;
				...
				return array("success"=> true, "msg"=>sprintf( __("Member access was removed from the %s",'wishlist-member'), $post["post_type"] ), "msg_type"=>"success", "data"=>$post );
			}
		}
		...
		function get_password_notification( $data ) {
			...
			if ( file_exists($f) ) {
				...
				include($f);
				$form = ob_get_clean();
				return array("success"=> true,"data"=> $data, "form"=>$form);
			} else {
				return array("success"=> false, "msg"=>__('Unable to retrieve the settings','wishlist-member'), "data"=> $data, "msg_type"=>"danger");
			}
		}
		...
		function create_system_page( $data ) {
			...
			if ( $post_if > 0 ) {
			    return array("success"=> false, "msg"=>__('The page you are trying to create already exists','wishlist-member'), "msg_type"=>"danger");
			}
			$page_data = array();
			...
			if ( $id ) {
				...
				return array("success"=> true, "post_id"=>$id, "data"=> $data, "post_title"=>$data['page_title'], "msg" => __('Page Created','wishlist-member'), "msg_type"=>"success");
			} else {
				return array("success"=> false, "msg"=>__('An error occured while creating the page','wishlist-member'), "msg_type"=>"danger");
			}
		}
		...
		function get_system_page( $data ) {
			...
			$pages["url"] = $pages_url;
			return array("success"=> true, "msg"=>__('System Page settings found','wishlist-member'), "msg_type"=>"success", "page_type"=>$page_type, "pages"=>$pages, "data"=>$data );
		}
		...
		function reset_custom_css( $data ) {
			...
			return array("success"=> true, "msg"=>__('CSS has been reset back to Default','wishlist-member'), "msg_type"=>"success", "css"=>$wlm_css );
		}
		...
		function save_membership_level($data) {
			...
			foreach(array('removeFromLevel', 'addToLevel') AS $option) {
			...
			return array("success"=> true, "msg"=>__('Saved','wishlist-member'), "msg_type"=>'success', "wpm_levels" => $x);
		}
		...
		function save_custom_registration_form($data) {
			...
			return array("success"=> true, "msg"=>__('Saved','wishlist-member'), "msg_type"=>'success', "regforms" => $regforms);
		}

		function clone_custom_registration_form($data) {
			$this->CloneCustomRegForm($data['id']);
			$regforms = $this->GetCustomRegForms();
			return array("success"=> true, "msg"=>__('Custom Registration Form Cloned','wishlist-member'), "msg_type"=>'success', "regforms" => $regforms);
		}
		...
		function delete_membership_level($data) {
			...
			if(empty($x[$data['id']]['count'])) {
				unset($x[$data['id']]);
				$this->SaveOption('wpm_levels',$x);
				return array("success"=> true, "msg"=>__('Membership Level Deleted','wishlist-member'), "msg_type"=>'warning', "wpm_levels" => $x);
			} else {
				return array("success"=> false, "msg"=>__('Cannot delete the Membership Level because it has members in it','wishlist-member'), "msg_type"=>'danger', "wpm_levels" => $x);
			}
		}

		function delete_custom_registration_form($data) {
			$this->DeleteCustomRegForm($data['id']);
			return array("success"=> true, "msg"=>__('Custom Registration Form Deleted','wishlist-member'), "msg_type"=>'warning', "wpm_regforms" => $this->GetCustomRegForms());
		}
		...
		function add_user($data) {
			...
			if ( !isset( $data[$password_fld] ) || empty( $data[$password_fld] ) ) {
				return array("success"=> false, "msg"=>__('Invalid passsword, please reload the page and try again','wishlist-member'), "msg_type"=>'danger', "data" => $data );
			}
			unset($data["password_field"]);
			$data["password1"] = $data["password2"] = $data[$password_fld];

			$send_welcome_email = (bool) wlm_arrval( $data, 'send_welcome_email' );
			$this->SaveOption( 'send_welcome_email', $send_welcome_email );
			
			unset( $data['send_welcome_email'] );
			$registered = $this->WPMRegister($data, $wpm_errmsg, $send_welcome_email, false);
			if ($registered) {
				return array("success"=> true, "msg"=>__('Member has been added','wishlist-member'), "msg_type"=>'success', "data" => $data );
			} else {
				return array("success"=> false, "msg"=>$wpm_errmsg, "msg_type"=>'danger', "data" => $data );
			}
		}
		...
		function delete_user($data) {
			...
			$return_data = array();
			foreach ( $userids as $id ) {
				if ( isset( $current_user->ID ) && $current_user->ID == $id ) continue;
				$x = wp_delete_user($id, 1);
				$return_data[$id] = $x;
			}
			$this->SyncMembership(true);
			if ( count( $userids ) > 1 ) {
				return array("success"=> true, "msg"=>__('Selected members have been deleted','wishlist-member'), "msg_type"=>'success', "data" => $return_data);
			} else {
				return array("success"=> true, "msg"=>__('Member has been deleted','wishlist-member'), "msg_type"=>'success', "data" => $return_data);
			}
		}
		...
		function update_user($data) {
			...
			$profileuser = $this->Get_UserData($data['userid']);
			if ( $operation != "get_form" ) {
				...
				if ( isset( $data['user_pass'] ) ) {
					$passmin = $this->GetOption( 'min_passlength' );
					$passmin+=0; if ( ! $passmin ) $passmin = 8;
				...
				if ( is_wp_error( $return ) ) {
					return array("success"=> false, "msg"=> $return->get_error_message(), "msg_type"=>"danger", "data"=> $data );
				}
				...
				if (!empty($user_custom_fields)) {
					...
					foreach ( $user_custom_fields AS $field => $v ) {
						if ( array_key_exists( $field, $custom_fields ) ) $this->Update_UserMeta($data['userid'], 'custom_' . $field, $v );
					}
				}
				...
				$level_data = "";
	            $wlUser = new WishListMemberUser($data['userid']);
	            $levels_count = count($wlUser->Levels);
	            if ( $levels_count ) {
		            wlm_add_metadata($wlUser->Levels);
		            $levels = $wlUser->Levels;
				...
				return array("success"=> true, "msg"=>__('Member profile has been updated','wishlist-member'), "msg_type"=>"success", "userdata"=> $data,'user_levels' => array_values($wlUser->Levels), "data"=> $return_data, );
			} else {
	        	if ( !$profileuser ) return array("success"=> false, "msg"=>__('Invalid member','wishlist-member'), "msg_type"=>"danger" );
				...
				return array("success"=> true, "msg"=>__('Member Found','wishlist-member'), "msg_type"=>"success","current_user" => $current_user->ID ,"data"=> $profileuser->data, "form"=>$edit_form);
			}
		}
		...
		function resend_reset_link( $data ) {
			...
			$this->RetrievePassword( $data['user_login'], true );
			return array("success"=> true, "msg"=>__('Password Reset link sent','wishlist-member'), "msg_type"=>"success", "data"=> $data);
		}
		...
		function logout_everywhere( $data ) {
			...
			return array("success"=> true, "msg"=>__('Member logged out','wishlist-member'), "msg_type"=>"success", "data"=> $data );
		}
		...
		function preview_broadcast( $data ) {
			...
			return array("success"=> true , "data"=>$data, "preview"=>$preview);
		}
		...
		function create_broadcast( $data ) {
			...
			$footer = array();
			if ( !empty( $signature ) ) $footer["signature"] = $signature;
			...
			if ( empty( $error ) ) {
				...
				if ( ! $record_id ) $error = __("An error occured while saving the broadcast.", 'wishlist-member') .$wpdb->last_error;
			}

			if (  !empty( $error ) ) {
				return array("success"=> false , "msg" => $error, "msg_type"=>"danger",  "data"=>$data );
			} else {
				return array("success"=> true , "msg" => __('Broadcast created','wishlist-member'), "msg_type"=>"success", "data"=>$data, "id"=>$record_id );
			}
		}

		function queue_broadcast( $data ) {
			$emailbroadcast = $this->GetEmailBroadcast( $data["id"] );
			if( !$emailbroadcast ) return array("success"=> false , "msg" => __('Invalid broadcast id','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
			...
			if ( $emailbroadcast->send_to == "send_mlevels" ) {
				...
				require_once( $this->legacy_wlm_dir . '/core/UserSearch.php');
			...
			$broadcast_data = array( "status"=> __('Queued','wishlist-member'), "total_queued"=>$total_queued);
			$this->UpdateEmailBroadcast( $data["id"], $broadcast_data );
			$data["total_queued"] = $total_queued;
			return array("success"=> true , "msg" => __('Your broadcast is already in queue','wishlist-member'), "msg_type"=>"success", "data"=>$data );
		}

		function get_email_broadcast( $data ) {
			$broadcast = $this->GetEmailBroadcast( $data["id"] );
			if ( $broadcast ) {
				if ( isset($broadcast->text_body) ) $broadcast->text_body = stripslashes($broadcast->text_body);
				if ( isset($broadcast->footer) ) $broadcast->footer = stripslashes($broadcast->footer);
				if ( isset($broadcast->subject) ) $broadcast->subject = stripslashes($broadcast->subject);
				return array("success"=> true , "msg" => __('Broadcast found','wishlist-member'), "msg_type"=>"success", "data"=>$data, "broadcast" => $broadcast);
			} else {
				return array("success"=> false , "msg" => __('Invalid broadcast id','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
			}
		}
		...
		function changestat_broadcast( $data ) {
			...
			$this->UpdateEmailBroadcast( $data["id"], $broadcast_data );
			return array("success"=> true , "msg" => __('Broadcast status updated','wishlist-member'), "msg_type"=>"success", "data"=>$data );
		}

		function delete_broadcast( $data ) {
			$this->DeleteEmailBroadcast( $data["id"] );
			return array("success"=> true , "msg" => __('Broadcast has been deleted','wishlist-member'), "msg_type"=>"success", "data"=>$data );
		}
		...
		function get_emails_in_queue($data) {
			...
			if ( count( $data ) > 0 && get_transient( 'wlm_is_sending_broadcast' ) === false ) $this->SendQueuedMail();

			return array("success"=> true , "msg" => __('Emails in Queue','wishlist-member'), "msg_type"=>"success", "data"=>$data, "cnt" => count( $data ) );
		}

		function send_emails_in_queue($data) {
			return array("success"=> $this->SendEmailQueue( $data["id"] ), "msg_type"=>"success", "data"=>$data );
		}
		...
		function get_broadcast_status($data) {
			...
			$html = ob_get_clean();
			return array("success"=> true , "data"=>$data, "html"=>$html);
		}
		...
		function get_backup_queue_count($data) {
			...
			if ( get_transient( 'wlm_is_doing_backup' ) === false ) $this->ProcessBackupQueue();

			return array("success"=> true , "msg" => __('Backup in Queue','wishlist-member'), "msg_type"=>"success", "data"=>$data, "cnt" => $queue_left, 'backup_monitor' => get_transient( 'wlm_backup_monitor' ) );
		}
		...
		function cancel_backup($data) {
			...
			if ( $queue ) {
				...
				foreach ( $queue as $q ) {
					$ids[] = $q->ID;
					$queue_val = maybe_unserialize($q->value);
				...
				return array("success"=> false , "msg" => __('No backup in queue to cancel','wishlist-member'), "msg_type"=>"danger" );
			}

			return array("success"=> true , "msg" => __('Backup has been cancelled','wishlist-member'), "msg_type"=>"success" );
		}
		...
		function get_import_queue_count($data) {
			...
			if ( $queue_count > 0 && get_transient( 'wlm_is_doing_import' ) === false ) $this->ProcessImportMembers();

			return array("success"=> true , "msg" => __('Import in Queue','wishlist-member'), "msg_type"=>"success", "data"=>$data, "cnt" => $queue_count );
		}
		...
		function pause_start_import($data) {
			...
			return array("success"=> true , "msg" => __('Import in Queue','wishlist-member'), "msg_type"=>"success", "data"=>$data );
		}
		...
		function cancel_member_import($data) {
			...
			if ( $queue ) {
				...
				return array("success"=> false , "msg" => __('No import in queue to cancel','wishlist-member'), "msg_type"=>"danger" );
			}

			return array("success"=> true , "msg" => __('Import has  been cancelled','wishlist-member'), "msg_type"=>"success" );
		}
		...
		function process_wizard($data){
			...
			return array_merge( $return, array("data"=> $data, "html"=>$html ) );
		}
		...
		function save_autoresponder($data) {
			...
			$ar[$id] = array_replace_recursive((array) $ar[$id], (array) $data);

			$this->SaveOption('Autoresponders', $ar);
			wp_send_json(array('success' => true, 'data' => $ar[$id]));
		}
		...
		function save_payment_provider($data) {
			...
			$_SESSION[__FUNCTION__] = 1;
			foreach($data AS $field => $value) {
				if(is_array($value) && $field != 'paypalec_spb') {
					$orig = $this->GetOption($field);
					if(empty($orig)) $orig = array();
					$value = array_replace_recursive($orig, $value);
				}
		...
		function get_content_protection($data) {
			...
			$html = ob_get_clean();
			return array('success' => true, 'data' => $data, 'html' => $html, 'content' => $content );
		}

		function update_content_protection($data) {
			$contentids = isset( $data["contentids"] ) ? $data["contentids"] : "";
			$contentids = explode(",", $contentids );
			if ( count( $contentids ) <= 0 ) return array("success"=> false, "msg"=>__('No content selected','wishlist-member'), "msg_type"=>"danger");
			...
			foreach ( $contentids as $contentid ) {
				...
				if ( ( isset( $data['wlm_levels'] ) && !empty( $data['wlm_levels'] ) ) || ( isset( $data['level_action'] ) && $data['level_action'] == "set" ) ) {
					...
					switch( $data['content_type'] ) {
						case 'categories':
							...
							if ( !$protect_inherit ) {
								if ( $action == "add" &&  !$protected ) $this->CatProtected( $contentid, 'Y' );
							}
							break;
						case 'folders':
							$protected = $this->FolderProtected( $contentid );
							if ( $action == "add" &&  !$protected ) $this->FolderProtected( $contentid, 'Y' );
							break;
						default:
							...
							if ( !$protect_inherit ) {
								if ( $action == "add" &&  !$protected ) $this->SpecialContentLevel( $contentid, 'Protection', 'Y', $x_content_type );
							} else {
				...
				if ( isset( $data['wlm_payperpost_users'] )  ) {
					...
					if ( !$post ) return array("success"=> false, "msg"=>"Invalid post", "msg_type"=>"danger", "data"=>$data);
					if ( !$user ) return array("success"=> false, "msg"=>"Invalid member", "msg_type"=>"danger", "data"=>$data);
					if ( $data['operation'] == "add" ) {
			...
			} else if ( $data['content_type'] == "folders" ) {
				$rootOfFolders = trim( $this->GetOption( 'rootOfFolders' ) );
				$folder_protection_full_path = $this->folder_protection_full_path( $rootOfFolders );
			...
			return array( 'success' => true, 'msg' => $msg, 'data' => $data, 'content' => $return_data );
		}
		...
		function enable_folder_protection ( $data ) {
			if ( !isset( $data['folder_protection'] )  ) return array('success' => false, "msg"=>__('Invalid setting','wishlist-member'), "msg_type"=>"danger", 'data' => $data );
			...
			if ( $data['folder_protection'] == "1" ) {
				...
				if ( !is_dir($rootoffolders) ) {
					...
					if ( !mkdir($rootoffolders) ) {
						return array("success"=> false, "msg"=>__('Folder Protection enabled but we were not able to create the folder on your host','wishlist-member'), "msg_type"=>"warning", "data"=>$data );
					}
				}
				if ( is_dir($rootoffolders) ) {
					$wpm_levels = $this->GetOption('wpm_levels');
				...
				$return = array("success"=> true, "msg"=>__('Folder Protection enabled','wishlist-member'), "msg_type"=>"success", "data"=>$data );
			} else {
				$this->RemoveAllHtaccessFromProtectedFolders();
				
				$return = array("success"=> true, "msg"=>__('Folder Protection disabled','wishlist-member'), "msg_type"=>"warning", "data"=>$data );
			}
			return $return;
		}
		...
		function folder_protection_autoconfig ( $data ) {
			...
			if ( $data['type'] == 'reset' ) {
				...
				if ( !is_dir($rootoffolders) ) {
					// if folder does not exist, we create it
					if ( !mkdir($rootoffolders) ) {
						return array('success' => false, "msg"=>__('Could not create folder','wishlist-member'), 'data' => $data );
					}
				}
				...
				return array('success' => false, "msg"=>__('Invalid operation','wishlist-member'), 'data' => $data );
				exit(0);
			}
			return array('success' => true, "msg"=>__('Done','wishlist-member'), 'data' => $data );
		}
		...
		function get_folders_list ( $data ) {
			...
			return array('success' => true, "msg"=>__('Done','wishlist-member'), 'folders' => $folders );
		}
		...
		function get_folders_files ( $data ) {
			...
			if ( $handle = opendir( $data['path'] ) ) {
				while (false !== ($entry = readdir($handle))) {
					...
					if ( !is_file( $data['path'] . '/' .$entry )  ) continue;
					if ( $entry == '.htaccess' ) continue;
					$files[] = $entry;
				}
				if ( count( $files ) <= 0 ) {
					return array('success' => false, "msg"=>__('Empty folder','wishlist-member'), 'data' => $data );
				}
			} else {
				return array('success' => false, "msg"=>__('Invalid folder','wishlist-member'), 'data' => $data );
			}
			return array('success' => true, "msg"=>__('Done','wishlist-member'), 'files' => $files );
		}
		...
		function enable_custom_post_types ( $data ) {
			...
			foreach ( $data as $key => $value) {
				...
				if ( $value == 1 ) {
					...
					if ( $k !== false ) unset( $enabled_types[$k] );
				}
				$enabled_types = array_unique( $enabled_types );
			}
			$this->SaveOption('protected_custom_post_types', $enabled_types );

			return array('success' => true, "msg"=>__('Custom Post Type protection updated','wishlist-member'), 'data' => $data );
		}
		...
		function update_user_profile( $data ) {
			...
			if( empty( $data['referrer'] ) ) $data['referrer'] = admin_url( 'profile.php' );

			$error = ['wlm_required' => []];
			if( !$data['nickname'] ) $error['wlm_required'][] = 'nickname';
			if( !$data['user_email'] ) $error['wlm_required'][] = 'user_email';
			...
			$udata = array_intersect_key( $data, array_flip( ['first_name', 'last_name', 'nickname', 'display_name', 'user_email'] ) );
			$udata['ID'] = $user->ID;

			wp_update_user( $udata );
			if( $data['new_pass'] ) {
				$passmin = ((int) $this->GetOption( 'min_passlength' ) ) ?: 8;
				if( strlen( $data['new_pass'] ) < $passmin || ( $this->GetOption( 'strongpassword' ) && !wlm_check_password_strength( $data['password1'] ) ) ) {
			...
			if( $data['profile_photo'] ) {
				$this->Update_UserMeta( $user->ID, 'profile_photo', $data['profile_photo'] );
			} else {
				$this->Delete_UserMeta( $user->ID, 'profile_photo' );
			}
		...
		function set_content_schedule ( $data ) {
			...
			if ( isset($data['post_option'] ) ) {
	            if ( $contentids[0] != "" && ( isset( $data['scheddays'] ) || isset( $data['hidedays'] ) ) ) {//save if theres post id
	                ...
	                $wlm_contentsched_Option = array(); $lvl_arr = array();
	                ...
	                return array( 'success'=> true, 'msg'=>__( 'Content schedule has been updated', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
	            } else {
	            	return array( 'success'=> false, 'msg'=>__( 'Content schedule was not updated', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
	            }
			} else {
				$wlm_levels = isset( $data["wlm_levels"] ) ? (array) $data["wlm_levels"] : array();
				if ( count( $wlm_levels ) <= 0 ) return array("success"=> false, "msg"=>__('No level selected','wishlist-member'), "msg_type"=>"danger");
				...
				if ( $data['sched_action'] == "set" ) {
					...
					return array( 'success'=> true, 'msg'=>__( 'Content schedule set', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
				} if ( $data['sched_action'] == "remove" ) {
					foreach ($wlm_levels as $key => $lvl) {
						$scheduler->DeleteContentSched($contentids, $lvl );
					}
					return array( 'success'=> true, 'msg'=>__( 'Content schedule has been removed', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
				}
			}
		}
		...
		function set_content_archive ( $data ) {
			...
			if ( isset($data['post_option'] ) ) {
                ...
                foreach ( (array)$wpm_levels AS $id => $level ) {
                    ...
                    if ( !$archiver->isvalid_date( $date, $post_ID ) ) {
                ...
                return array( 'success'=> true, 'msg'=>__( 'Content archive date has been updated', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
			} else {
				$wlm_levels = isset( $data["wlm_levels"] ) ? (array) $data["wlm_levels"] : array();
				if ( count( $wlm_levels ) <= 0 ) return array("success"=> false, "msg"=>__('No level selected','wishlist-member'), "msg_type"=>"danger");
				...
				if ( $data['sched_action'] == "set" ) {
					...
					return array( 'success'=> true, 'msg'=>__( 'Content archive date set', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
				} elseif ( $data['sched_action'] == "remove" ) {
					foreach ($wlm_levels as $key => $lvl) {
						$archiver->DeletePostExpiryDate($contentids, $lvl );
					}
					return array( 'success'=> true, 'msg'=>__( 'Content archive date has been removed', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
				}
			}
		}
		...
		function set_content_manager ( $data ) {
			...
			if ( $data['sched_action'] == "set" ) {
				...
				if( $datenow > $schedule_date ) return array( 'success'=> false, 'msg'=>__( 'Invalid date', 'wishlist-member' ), 'msg_type' => 'danger', 'data' => $data);

				$d = [];
				if ( $data['content_action'] == "move" || $data['content_action'] == "add" ) {
					$cats = isset( $data["content_cat"] ) ? (array) $data["content_cat"] : array();
					if ( count($cats) <= 0 ) return array( 'success'=> false, 'msg'=>__( 'Please select a category', 'wishlist-member' ), 'msg_type' => 'danger', 'data' => $data);
					$cats = implode("#", $cats);
					$d = [
						"action" => "move",
					...
					if ( empty($data['content_status']) ) return array( 'success'=> false, 'msg'=>__( 'Invalid post status.', 'wishlist-member' ), 'msg_type' => 'danger', 'data' => $data);
					$d = [
						"action" => "set",
						"method" => $data['content_action'],
						"date" => $schedule_date,
				...
				if ( isset($data['post_option']) ) {
		            ...
		            switch ($d["action"]) {
		                case 'move':
		                    if ( $d["method"] == "move" ) $str_sched = "Move to ";
		                    else $str_sched = "Add to ";
		                    $cat = explode('#',$d["cats"]);
		                    $t = [];
				...
				if ( $schedid ) {
					...
					return array( 'success'=> true, 'msg'=>__( 'Content schedule has been updated', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data, 'action_type' =>$d['action'], 'str_sched' => $str_sched, 'insertid' => $schedid );
				} else {
					$insertid = $manager->SavePostManagerDate( $contentids, $d );
					return array( 'success'=> true, 'msg'=>__( 'Content schedule date set', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data, 'action_type' =>$d['action'], 'str_sched' => $str_sched, 'insertid' => $insertid );
				}
			} elseif ( $data['sched_action'] == "remove" ) {
				...
				if ( isset($data['post_option']) ) {
					...
					return array( 'success'=> true, 'msg'=>__( 'Schedule of the content has been removed', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
				} else {
					if ( $schedid ) {
						$manager->DeletePostManagerDate($schedid,$data['content_action']);
						return array( 'success'=> true, 'msg'=>__( 'The schedule has been removed', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
					} else {
						$content_action = $data['content_action'] == "add" ? "move" : $data['content_action'];
						if ( ! in_array( $content_action, $sched_type ) ) return array( 'success'=> false, 'msg'=>__( 'Invalid action to remove', 'wishlist-member' ), 'msg_type' => 'danger', 'data' => $data);
		...
		function get_contentcontrol_settings( $data ) {
			...
			if ( $type == "scheduler" ) {
				...
				return array("success"=> false, "msg"=>__('Invalid settings','wishlist-member'), "msg_type"=>"danger", "data"=>$data );
			}

			$page_internal = $this->GetOption( $type .'_error_page_internal' );
			if ( !$page_internal) $page_internal = $page_type &&  $page_type != "url" && $page_type != "internal" && $page_type != "text" ? $page_type  : false;
			...
			return array("success"=> true, "msg"=>__('Content Scheduler settings found','wishlist-member'), "msg_type"=>"success", "settings"=>$settings, "data"=>$data );
		...
		function reset_level_sender_info_to_default() {
			...
			return array( 'success'=> true, 'msg'=>__( 'Sender Info Reset for All Levels', 'wishlist-member' ), 'msg_type' => 'success', 'data' => $data);
		}
		...
		function apply_email_template_to_selected_levels( $data ) {
			...
			if( empty( $data['content']) || empty( $data['levels'] ) ) {
				return array( 'success'=> false, 'msg' => 'Invalid data', 'msg_type' => 'danger' );
			}
			...
			return array( 'success'=> true, 'msg' => $msg, 'msg_type' => 'success', 'data' => $data);
		}
	}
}
